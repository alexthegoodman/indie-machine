...
- `EntropyApp::with_art_assets_project(id)` and a "Global"-tagged `Entropy.Model.load` - a bare `EntropyApp` couldn't load `.glb` models at all before this; flat hand-authored boxes read the shader differences far more weakly than a textured character does.
