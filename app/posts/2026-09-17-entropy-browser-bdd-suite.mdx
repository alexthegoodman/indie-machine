...
- **`tests/features/browser_live.feature`** (`@live-ui`) - the same shape of scenarios, written to document what the live tier actually does to the real app. See the Decision Log for why "document" is the operative word.
...
The full scripted sequence for this addon:
...
actions: VecDeque::from([
    ...
    // ...follow a link, go back, go forward, capture "history"...
    // ...bookmark, capture "bookmark"...
    // ...load an invalid URL, capture "error"...
...
**`browser_live.feature` documents the live tier; it doesn't drive it.** This is worth stating plainly rather than leaving implicit: `tests/browser_bdd.rs` only ever calls `BrowserWorld::run` on `browser_loop.feature`. The `@live-ui` scenarios in `browser_live.feature` are hand-written Gherkin describing the same sequence `BrowserBddDriver`'s hardcoded `VecDeque` of actions happens to perform - readable documentation of intent, not a machine-enforced contract. Nothing today would catch the two drifting apart if someone edited the driver's action list without also updating the `.feature` file, or vice versa. That's a real gap in this session's own definition of "BDD," not a hypothetical one.

## What's next

- Reconcile `browser_live.feature` and `BrowserBddDriver` so the feature file actually generates (or at minimum validates) the driver's action sequence, closing the gap above. Tracked as a new backlog card, `browser-live-feature-driver-sync`.
