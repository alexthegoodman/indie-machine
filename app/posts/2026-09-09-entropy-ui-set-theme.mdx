...
`examples/studio-bundle/src/theme_gallery_addon.ts` + `src/bin/example_theme_gallery.rs`, following the same minimal-bin pattern as the [media player example](/posts/entropy-media-player) - all the logic lives in the addon, the bin is three lines:
...
- Wire `setTheme` into `EntropyApp` itself (a `.with_theme(ThemeDescriptor)` builder method, mirroring `.with_title()`/`.with_window_icon()` from the [window-config post](/posts/entropy-app-window-config)), so an embedder can set a starting theme before any addon code runs, instead of every addon having to call `setTheme` from its own `onInit`.
